...
def compute_tSNE_2d(df_ROI_final, features_options='basic', perplexity=15, color="species",early_exag=12 ,learning_rate=200,n_iter=1000,method="barnes_hut"):
    # 2D tSNE-----------------------------------------------
    ...
    fig = px.scatter(
        projections, x=0, y=1,
        color=df[color], labels={'color': 'species'}
    )
    return fig


def compute_tSNE_3d(df_ROI_final, features_options='basic', dimensions=2, color="species"):
