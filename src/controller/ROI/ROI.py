...
wavfilepath = '/Users/Paul/Paul/Desktop/My_projects/Bioacoustics/BioSound_Maputo/datasets/xenocanto/wav/Crithagra_sulphurata/31660.wav'


def find_ROIs_soundfile(path=wavfilepath, display=False, mode_bin='relative', param1=10, param2=0.9, fmin=100, fmax=15000):
    ...
    if display:
        ...
        fig, ax = maad.util.plot2d(
            Sxx_db_noNoise, title='Power spectrogram density (PSD)_NoNoise', **fig_kwargs)
        fig, ax = maad.util.plot2d(
            Sxx_db_noNoise_smooth, title='Power spectrogram density (PSD)_NoNoise +smooth', **fig_kwargs)
...
def compute_ROI_all_features(s, fs, df_rois, resolution='med', display=True):
    ...
    for index, row in df_rois.iloc[1:].iterrows():
        ...
        # to the current ROI
        s_trim = sound.trim(
            s, fs, row.loc[index, 'min_t'], row.loc[index, 'max_t'])
...
def compute_Sxx_dB_nonoise_smooth(path=wavfilepath, fmin=100, fmax=10000, smoothing=0.5):
    # workaround RIFF bug from xenocanto import TO BE CHANGED
    ...
    # Then we smooth the spectrogram in order to facilitate the creation of masks as
    # small sparse details are merged if they are close to each other
    Sxx_db_noNoise_smooth = sound.smooth(Sxx_db_noNoise, std=smoothing,
                                         display=False, savefig=None,
                                         **{'vmin': 0, 'vmax': dB_max, 'extent': ext})
    return Sxx_db_noNoise_smooth, tn, fn, ext


def ROI_and_centroid(Sxx_db_noNoise_smooth, tn, fn, ext, mode_bin='relative', param1=18, param2=0.7, display=False):
    # Then we create a mask (i.e. binarization of the spectrogram) by using the
    # double thresholding technique
    im_mask = rois.create_mask(im=Sxx_db_noNoise_smooth, mode_bin=mode_bin,
                               bin_std=param1, bin_per=param2,
                               verbose=False, display=False)
    ...
    if df_rois.empty:
        ...
        print('number of rois : %d' % len(df_rois))
